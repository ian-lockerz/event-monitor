...
import com.lockerz.kestrel.ClientException;
...
public class Client implements AsynchronousClient {
    ...
    public void init() throws IOException {
        ...
        Runnable queueWorker = new QueueWorker(workQueue, socketWorker);
    ...
    public void setAndForget(String queueName, long expiration, byte[] data) {
        log.trace("setAndForget");
        try {
            set(queueName, expiration, data, new LoggingSetResponseHandler());
        } catch (IOException e) {
            log.error("Swallowing exception caught while setting and forgetting [{}]",
                    e.getMessage(), e);
        }
    }
        ...
        throws IOException
    {
        ...
        if (!this.workQueue.offer(WorkItem.get(
    ...
    void closeGet(String queueName) throws IOException {
        log.trace("closeGet");
        StringBuilder buf = new StringBuilder();
        buf.append("get ").append(queueName).append("/close").append("\r\n");
        if (this.workQueue.offer(WorkItem.maybeSend(
                buf.toString().getBytes(), new InternalCloseGetHandler())))
        {
            // TODO handle this case
            log.warn("Overloaded - could not close get");
        }
    }

    void restart(WorkItem item) {
        log.debug("Restarting [{}]", item.getOp());
        if (!this.workQueue.offer(item)) {
            item.getHandler().onError("OVERLOADED", "Work queue is overloaded");
        }
    }
                ...
                try {
                    ...
                    try {
                        switch (item.getOp()) {
                            ...
                            case GET : this.socketWorker.sendGet(item); break;
    ...
    private static class SocketWorker implements Runnable {
        ...
        public void sendSet(WorkItem item) throws IOException {
            ...
            try {
                ...
                log.trace("Waking up selector - sendSet");
        ...
        public void sendGet(WorkItem item) throws IOException {
            ...
            try {
                ...
                log.trace("Waking up selector - sendGet");
                this.selector.wakeup();
            } catch (NoSuchElementException e) {
                throw new SocketPoolExhaustedException(e);
            } catch (IOException e) {
        ...
        public void maybeSend(WorkItem item) throws IOException {
            try {
                log.trace("maybeSend");
                SocketChannel channel = (SocketChannel) this.socketPool.borrowObject();
                if (this.activeWorkItems.put(channel, item) != null) {
                ...
                log.trace("Waking up selector - maybeSend");
                this.selector.wakeup();
            } catch (NoSuchElementException e) {
                throw new SocketPoolExhaustedException(e);
            } catch (IOException e) {
    ...
    private static class WorkItem {
        public static enum Op { SET, GET, MAYBE_SEND; }
